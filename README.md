...
An unofficial, noncommercial, open-source companion for playing the 1981 **Dark Tower** board game with modern Windows, macOS, and tablet hardware.
...
## Current release: 0.10.1 — Apple Silicon macOS support

Version **0.10.1** provides:

- a Windows portable executable;
- a Windows NSIS setup executable;
- a native Apple Silicon macOS app and DMG;
...
Version 0.10.1 adds a native Apple Silicon build intended for M1, M2, M3, M4, and newer Apple Silicon Macs. GitHub Actions builds both an application bundle and a DMG installer on an ARM64 macOS runner.

The test build uses ad-hoc signing rather than an Apple Developer certificate, so macOS may require an explicit first-launch approval in Privacy & Security. A future public release can add Developer ID signing and notarization.
