...
## 0.10.1 — macOS Apple Silicon support

- Adds a GitHub Actions workflow for native Apple Silicon (`arm64`) macOS builds.
- Produces both a `.app` ZIP and a drag-to-Applications `.dmg`.
- Adds a macOS `.icns` application icon.
- Uses ad-hoc macOS signing so personal test builds can run on Apple Silicon without an Apple Developer certificate.
- Keeps the native Shared Tower Display window available on macOS.
- Updates the Windows workflow to the same v0.10.1 source.
- No gameplay, probability, movement, map, or balance changes.

## 0.10.0 — Milestone 9 map refinement and native shared display

- Replaces the Windows `window.open` popup with a native Tauri Shared Tower window.
- Keeps browser/iPad popup behavior as a fallback.
- Recreates the native display window if the user previously closed it.
- Rolls the board geometry back to a stable non-crossing shared-boundary mesh.
- Removes tiny sliver territories and boundary self-intersections.
- Widens the Bazaar, Ruin, Sanctuary, Tomb, and Dark Tower regions.
- Adds hand-tuned diagonal label placement for every reported problem area.
- Displays DARK and TOWER on two lines in all four Dark Tower spaces.
- Moves every Citadel shield farther outside the board.
- Replaces the primitive crest drawings with more detailed original heraldic vectors.
- Preserves the 40/30/30 default workspace and all movement rules.

## 0.9.0 — Milestone 8 board fidelity and heraldry

- Changes the default desktop workspace to 40% Map, 30% Tower, and 30% Dashboard.
- Replaces regular ring-sector territory shapes with a shared irregular mesh of wavy radial and curved ring boundaries.
- Adds a scalloped outer board edge inspired by the original physical board.
- Rotates and adaptively fits special-location names inside their territories.
- Shows the complete territory name in the map status ribbon on hover or keyboard focus.
- Moves Citadel heraldry outside the board beside each Citadel.
- Adds newly drawn Lion, Griffin, Eagle, and Unicorn house emblems.
- Expands the SVG viewBox so external Citadel shields are never clipped.
- Adds offline caching for the board and workspace modules.

## 0.8.0 — Milestone 7 adaptive workspace

- Opens the Windows app maximized while keeping the normal window frame and taskbar.
- Makes the tabletop height follow the available native-window height.
- Adds draggable Map/Tower, Tower/Dashboard, and Player/Chronicle dividers.
- Saves custom panel proportions locally.
- Adds Balanced, Map Focus, Tower Focus, Dashboard Focus, and Equal Columns presets.
- Uses container-relative sizing so Tower controls and map artwork scale with their panels.
- Docks the guided tour beside the game on desktop instead of covering it.
- Reserves bottom space for the tour on narrower windows.

## 0.7.0 — Milestone 6 full board and movement

- Reconstructed the original circular board as 96 interactive spaces: 23 territories per kingdom plus four Frontier territories.
- Preserved the original relative placement of Citadels, Sanctuaries, Bazaars, Tombs, Ruins, Dark Tower spaces, and counterclockwise Frontiers.
- Added adjacency highlighting, automatic illegal-move prevention, Dragon blocking, Lost movement, Pegasus destinations, zoom, and panning.
- Added save migration from the prior kingdom-only position model.
- Retained a manual-board setting for groups using a physical board.

## 0.6.0 — Milestone 5 iPad and shared display

- Added a tablet portrait panel switcher for Map, Tower, Player, and Chronicle.
- Added iPad safe-area spacing and larger coarse-pointer touch targets.
- Added a Device & iPad panel with installation guidance.
- Added share-sheet save export with a normal download fallback.
- Added optional screen wake lock and full-screen controls.
- Added a synchronized display-only Tower window for a second monitor or screen mirroring.
- Added platform feature detection and automated tests.
