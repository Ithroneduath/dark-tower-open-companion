...
import { boardMoveCheck, citadelSpaceId, frontierSpaceId, kingdomForSpace, legalBoardMoveIds, pegasusDestinationIds, spaceAction, spaceById } from "./board.js";
...
export function createGame({
  ...
  const game = {
    ...
    board: { dragonSpaceId: null, locationActionUsed: false },
...
function createPlayer(name, index) {
  ...
  return {
    ...
    currentSpaceId: citadelSpaceId(homeKingdom),
    previousSpaceId: citadelSpaceId(homeKingdom),
    warriors: 10,
    gold: 30,
    food: 25,
    scout: false,
...
export function setPlayerKingdom(game, kingdom) {
  ...
  player.previousSpaceId = player.currentSpaceId;
  player.currentSpaceId = citadelSpaceId(kingdom);
  if (game.board) game.board.locationActionUsed = false;
  logEvent(game, `${player.name}'s board marker was corrected to the ${kingdom} Citadel.`);
  touch(game);
}


export function legalMovesForCurrentPlayer(game) {
  assertPlaying(game);
  return legalBoardMoveIds(game, currentPlayer(game));
}

export function pegasusMovesForCurrentPlayer(game) {
  assertPlaying(game);
  return pegasusDestinationIds(game, currentPlayer(game));
}

export function enterBoardSpace(game, destinationId, { viaPegasus = false } = {}) {
  assertReadyForLocation(game);
  const player = currentPlayer(game);
  const destination = spaceById(destinationId);
  if (!destination) throw new RangeError("Unknown board territory.");
  const check = boardMoveCheck(game, player, destinationId, { pegasus: viaPegasus });
  if (!check.ok) throw new Error(check.reason);

  const originId = player.currentSpaceId;
  const originKingdom = player.currentKingdom;
  if (destination.type === "frontier") {
    const result = resolveFrontier(game, destination.to);
    if (result.type === "crossed") {
      player.previousSpaceId = originId;
      player.currentSpaceId = destination.id;
    }
    game.board.locationActionUsed = true;
    touch(game);
    return { ...result, spaceId: destination.id };
  }

  if (destination.id !== originId) {
    player.previousSpaceId = originId;
    player.currentSpaceId = destination.id;
    player.previousKingdom = originKingdom;
    player.currentKingdom = destination.kingdom;
    logEvent(game, `${player.name} moved to ${destination.label}.`);
  } else {
    logEvent(game, `${player.name} remained at ${destination.label}.`);
  }
  game.board.locationActionUsed = true;

  let result;
  switch (destination.action) {
    case "move": result = resolveMove(game); break;
    case "bazaar": result = startBazaar(game); break;
    case "tomb": result = resolveTombRuin(game); break;
    case "sanctuary": result = resolveSanctuaryOrCitadel(game, destination.type === "citadel" ? "citadel" : "sanctuary"); break;
    case "darktower": result = beginDarkTower(game); break;
    default: throw new Error(`Unhandled board action: ${destination.action}`);
  }
  touch(game);
  return { type: "entered", destination: destination.id, action: destination.action, result };
}

export function usePegasusToSpace(game, destinationId) {
  assertReadyForLocation(game);
  const player = currentPlayer(game);
  if (!player.pegasus) return { type: "unavailable" };
  if (player.currentKingdom !== player.homeKingdom && !player.keysByKingdom[player.currentKingdom]) {
  ...
  const destination = spaceById(destinationId);
  if (!destination) throw new RangeError("Unknown Pegasus destination.");
  player.pegasus = false;
  const result = enterBoardSpace(game, destinationId, { viaPegasus: true });
  logEvent(game, `${player.name} used Pegasus to reach ${destination.label}.`);
  touch(game);
  return { type: "used", destination: destinationId, result };
}
...
export function resolveFrontier(game, destinationKingdom) {
  ...
  player.previousSpaceId = player.currentSpaceId;
  player.currentSpaceId = frontierSpaceId(leaving);
...
function resolveLost(game) {
  ...
  player.skipTurns += 1;
  if (player.previousSpaceId && spaceById(player.previousSpaceId)) {
    const lostFrom = player.currentSpaceId;
    player.currentSpaceId = player.previousSpaceId;
    player.previousSpaceId = lostFrom;
    player.currentKingdom = kingdomForSpace(player.currentSpaceId, player.currentKingdom);
  }
...
function resolveDragon(game) {
  ...
  if (player.sword) {
    ...
    if (game.board) game.board.dragonSpaceId = null;
  ...
  if (game.board) game.board.dragonSpaceId = player.currentSpaceId;
...
export function showInventory(game) {
  assertReadyForLocation(game);
  if (game.board) game.board.locationActionUsed = true;
  const player = currentPlayer(game);
...
export function importGame(raw) {
  ...
  migrateBoardState(parsed);
  validateGame(parsed);
  parsed.updatedAt = new Date().toISOString();
  return parsed;
}

function migrateBoardState(game) {
  if (game.schemaVersion === 2) game.schemaVersion = 3;
  game.board ||= { dragonSpaceId: null, locationActionUsed: false };
  game.board.dragonSpaceId ??= null;
  game.board.locationActionUsed = Boolean(game.board.locationActionUsed);
  for (const player of game.players || []) {
    player.currentSpaceId ||= citadelSpaceId(player.currentKingdom || player.homeKingdom);
    player.previousSpaceId ||= player.currentSpaceId;
  }
}
...
export function validateGame(game) {
  ...
  if (game.schemaVersion !== 3) throw new Error(`Unsupported save schema: ${game.schemaVersion}`);
